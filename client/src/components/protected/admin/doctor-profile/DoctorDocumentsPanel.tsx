    ...
    <article className="rounded-2xl border border-gray-100 bg-white p-4 shadow-sm transition hover:border-blue-100 hover:shadow-md">
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h3 className="text-sm font-bold text-gray-950">{documentTypeLabel(document.documentType)}</h3>
            <p className="mt-1 text-xs text-gray-500">Uploaded {formatDate(document.uploadedAt)}</p>
          </div>
        </div>
      ...
      {document.remarks && <p className="mt-3 rounded-xl bg-gray-50 p-3 text-xs leading-5 text-gray-600">{document.remarks}</p>}

      <a
        href={document.fileUrl}
        target="_blank"
        rel="noopener noreferrer"
        className="mt-4 inline-flex items-center gap-2 rounded-lg border border-gray-200 px-3 py-2 text-xs font-semibold text-gray-700 transition hover:border-blue-200 hover:bg-blue-50 hover:text-blue-700"
      >
...
export function DoctorDocumentsPanel({ documents }: { documents: DoctorDocumentDTO[] }) {
  return (
    <SectionCard title="Verification Documents" description="Documents submitted by the doctor for admin review.">
      {documents.length === 0 ? (
        <div className="rounded-xl border border-dashed border-gray-200 bg-gray-50 p-6 text-sm text-gray-500">No documents have been submitted.</div>
      ) : (
